...
{} = cv2.imdecode({}, cv2.IMREAD_UNCHANGED)
	...
	snippet(
		"imwrite1",
		fmt("cv2.imencode({}, {})[1].tofile({})", { insert(1, "suffix"), insert(2, "image"), insert(3, "img_file") })
	),
	snippet("bgr2rgb", fmt("{} = cv2.cvtColor({}, cv2.COLOR_BGR2RGB)", { insert(1, "image"), fn(same_as, 1) })),
	snippet("rgb2gray", fmt("{} = cv2.cvtColor({}, cv2.COLOR_RGB2GRAY)", { insert(1, "image"), fn(same_as, 1) })),
	snippet("rgb2bgr", fmt("{} = cv2.cvtColor({}, cv2.COLOR_RGB2BGR)", { insert(1, "image"), fn(same_as, 1) })),
